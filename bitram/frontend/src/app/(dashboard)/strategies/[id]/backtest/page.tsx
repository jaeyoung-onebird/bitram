...
function StatCard({
  ...
  return (
    <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg text-center relative">
      <div className="flex items-center justify-center gap-1 mb-1">
        ...
        {tooltip && (
          ...
          >
            <span className="w-3.5 h-3.5 rounded-full bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400 text-[9px] font-bold flex items-center justify-center cursor-default select-none">?</span>
            {show && (
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 bg-slate-900 dark:bg-slate-950 text-white text-[11px] leading-relaxed rounded-lg px-3 py-2 shadow-xl z-50 pointer-events-none text-left">
                {tooltip}
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
          ...
          {bt && (
            <button
              onClick={() => router.push(`/community/new?from=backtest&strategy_id=${strategyId}`)}
              className="flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm text-white bg-blue-500 hover:bg-blue-600 rounded-lg transition"
            >
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <StatCard
              ...
              tooltip="백테스트 기간 동안 전략이 달성한 총 수익률. 초기 자본 대비 최종 자본의 증감률입니다."
            />
            <StatCard
              label="벤치마크 수익률"
              ...
              sub="Buy & Hold"
              tooltip="같은 기간 동안 단순히 매수 후 보유(Buy & Hold)했을 때의 수익률. 전략이 이 수치를 넘어야 의미 있습니다."
            />
            <StatCard
              label="승률"
              ...
              tooltip="전체 거래 중 수익을 낸 거래의 비율. 50% 이상이면 절반 이상 수익, 하지만 승률만으로 좋은 전략인지는 판단할 수 없습니다."
            />
            <StatCard
              label="최대 낙폭 (MDD)"
              value={`${bt.max_drawdown_pct.toFixed(2)}%`}
              color="orange"
              tooltip="고점 대비 최대 하락 폭. 낮을수록 리스크가 적습니다. 20% 이하면 양호, 30% 초과 시 리스크가 큰 전략입니다."
            />
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <StatCard
              ...
              tooltip="위험 대비 수익률 지표. 1.0 이상이면 양호, 2.0 이상이면 우수합니다. 높을수록 같은 위험에서 더 많은 수익을 냅니다."
            />
            <StatCard
              label="Profit Factor"
              value={bt.profit_factor.toFixed(2)}
              color={bt.profit_factor >= 1.5 ? "green" : bt.profit_factor >= 1 ? "gray" : "red"}
              tooltip="총 수익 ÷ 총 손실. 1.0 이상이면 수익, 1.5 이상이면 우수, 2.0 이상이면 매우 우수한 전략입니다."
            />
            <StatCard
              label="평균 수익 / 손실"
              value={`+${bt.avg_profit_pct.toFixed(2)}% / ${bt.avg_loss_pct.toFixed(2)}%`}
              color="purple"
              tooltip="수익 거래의 평균 수익률과 손실 거래의 평균 손실률. 평균 수익이 평균 손실보다 클수록 유리한 전략입니다."
            />
            <StatCard
              label="평균 보유 기간"
              ...
              tooltip="한 거래를 유지한 평균 캔들(봉) 수. 선택한 타임프레임 기준이며, 값이 클수록 장기 보유 전략입니다."
            />
          </div>

...
import type { Strategy, BacktestResult, CommunityBoard, PostSeriesItem } from "@/types";
...
function deriveCategoryFromSlug(slug: string | null): string {
  if (!slug) return "free";
  const s = slug.toLowerCase();
  if (s.includes("strategy") || s.includes("전략")) return "strategy";
  if (s.includes("question") || s.includes("질문") || s.includes("qa")) return "question";
  if (s.includes("profit") || s.includes("수익")) return "profit";
  if (s.includes("chart") || s.includes("차트")) return "chart";
  if (s.includes("news") || s.includes("뉴스")) return "news";
  if (s.includes("humor") || s.includes("유머")) return "humor";
  return "free";
}

function buildBacktestMarkdown(s: Strategy, bt: BacktestResult): string {
  const sign = (n: number) => (n >= 0 ? "+" : "");
  return `## ${s.name} 백테스트 분석 결과

**페어:** ${s.pair} | **타임프레임:** ${s.timeframe}
**백테스트 기간:** ${bt.start_date} ~ ${bt.end_date}

---

## 핵심 성과

- **총 수익률:** ${sign(bt.total_return_pct)}${bt.total_return_pct.toFixed(2)}% (벤치마크 ${sign(bt.benchmark_return_pct)}${bt.benchmark_return_pct.toFixed(2)}%)
- **승률:** ${bt.win_rate.toFixed(1)}% (${bt.win_trades}승 / ${bt.lose_trades}패, 총 ${bt.total_trades}거래)
- **최대 낙폭(MDD):** ${bt.max_drawdown_pct.toFixed(2)}%
- **Sharpe Ratio:** ${bt.sharpe_ratio.toFixed(2)}
- **Profit Factor:** ${bt.profit_factor.toFixed(2)}
- **평균 수익/손실:** ${sign(bt.avg_profit_pct)}${bt.avg_profit_pct.toFixed(2)}% / ${bt.avg_loss_pct.toFixed(2)}%

---

## 전략 설명

(이 전략에 대한 설명을 작성해주세요. 진입 조건, 청산 조건, 주의사항 등)
`;
}
...
function NewPostForm() {
  ...
  const fromBacktest = searchParams.get("from") === "backtest";
  const backtestStrategyId = searchParams.get("strategy_id");
  ...
  const [mySeries, setMySeries] = useState<PostSeriesItem[]>([]);
  const [submitting, setSubmitting] = useState(false);

  // Category is derived from board slug — no user selection needed
  const category = fromBacktest ? "strategy" : deriveCategoryFromSlug(boardSlug);
  ...
  useEffect(() => {
    ...
    if (fromBacktest) {
      // Auto-fill from backtest result
      if (backtestStrategyId) {
        api.getStrategy(backtestStrategyId)
          .then((s) => {
            setStrategyId(backtestStrategyId);
            setContentFormat("markdown");
            if (s.backtest_result) {
              setTitle(`[전략공유] ${s.name} 백테스트 결과 (${s.backtest_result.total_return_pct >= 0 ? "+" : ""}${s.backtest_result.total_return_pct.toFixed(2)}%)`);
              setContent(buildBacktestMarkdown(s, s.backtest_result));
            } else {
              setTitle(`[전략공유] ${s.name}`);
            }
          })
          .catch(console.error);
      }
      // Try to load strategy board; fall back to first available
      api.getCommunity("strategy")
        .then(setBoard)
        .catch(() =>
          api.getCommunities()
            .then((bs) => { if (bs.length) setBoard(bs[0]); })
            .catch(console.error)
        )
    ...
    api.getCommunity(boardSlug)
      ...
      .finally(() => setBoardLoading(false));
  }, [fromBacktest, backtestStrategyId, boardSlug, router]);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        <Link
          href={
            fromBacktest && backtestStrategyId
              ? `/strategies/${backtestStrategyId}/backtest`
              : boardSlug
              ? `/community/boards/${boardSlug}`
              : "/community"
          }
        ...
        <div>
          <h1 className="text-xl font-bold">
            {fromBacktest ? "백테스트 결과 공유" : "새 글 작성"}
          </h1>
          {fromBacktest ? (
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">백테스트 결과가 자동으로 채워졌어요. 내용을 수정한 뒤 게시해주세요.</p>
          ) : board && (
      ...
      <div className="space-y-6">
        ...
        {mySeries.length > 0 && (
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-600 dark:text-slate-300">시리즈 (선택)</label>
            <select
              value={seriesId}
              onChange={(e) => setSeriesId(e.target.value)}
              className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm sm:text-base text-slate-800 dark:text-slate-100 focus:outline-none focus:border-blue-500 transition appearance-none"
            >
              <option value="">시리즈에 포함하지 않음</option>
              {mySeries.map((s) => (
                <option key={s.id} value={s.id}>
                  {s.title} ({s.post_count}편)
                </option>
              ))}
            </select>
          </div>

...
function CommunityContent() {
  ...
  const [copiedPostIds, setCopiedPostIds] = useState<Set<string>>(new Set());
  ...
  const handleCopyStrategy = async (e: React.MouseEvent, postId: string) => {
    e.preventDefault();
    e.stopPropagation();
    if (copyingPostId || copiedPostIds.has(postId)) return;
    setCopyingPostId(postId);
    try {
      await api.copyStrategyFromPost(postId);
      setCopiedPostIds((prev) => new Set([...prev, postId]));
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col items-end gap-1.5 text-xs text-slate-400 dark:text-slate-500 shrink-0 pt-0.5">
                        {post.has_strategy && (
                          <button
                            onClick={(e) => handleCopyStrategy(e, post.id)}
                            disabled={copyingPostId === post.id || copiedPostIds.has(post.id)}
                            className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold transition ${copiedPostIds.has(post.id) ? "bg-emerald-500/10 text-emerald-500" : "bg-blue-500/10 text-blue-500 hover:bg-blue-500/20"} disabled:opacity-50 disabled:cursor-not-allowed`}
                          >
                            {copiedPostIds.has(post.id) ? (
                              <>
                                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                                복사됨
                              </>
                            ) : copyingPostId === post.id ? (
                              "복사 중..."
                            ) : (
                              <>
                        ...
                        <span className="flex items-center gap-1">
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M7.5 15h2.25m8.024-9.75c.011.05.028.1.037.15.544 2.913-.997 5.768-3.524 6.736a.44.44 0 00-.278.447c.044 1.39.156 2.53.354 3.417H5.25a2.25 2.25 0 01-2.25-2.25v-.894c0-.796.42-1.534 1.105-1.937A5.23 5.23 0 006.3 7.66c-.046-.38-.07-.765-.07-1.155A5.505 5.505 0 0111.735 1c2.243 0 4.203 1.348 5.063 3.276.138.31.245.632.318.966zM16.5 15h1.875a.625.625 0 01.625.625v5.25a.625.625 0 01-.625.625H16.5v-6.5z" /></svg>
                          {post.like_count}
                        </span>
                        <span className="flex items-center gap-1">
          ...
          {!loading && posts.length > 0 && (
            <div className="flex items-center justify-center gap-1.5 pt-2 flex-wrap">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}
                className="flex items-center gap-1 px-3 py-2 text-sm font-medium rounded-xl border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-800 dark:hover:text-slate-100 disabled:opacity-30 disabled:cursor-not-allowed transition"
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                이전
              </button>
              {Array.from({ length: (hasMore ? page + 2 : page) - Math.max(1, page - 4) + 1 }, (_, i) => Math.max(1, page - 4) + i).map((p) => (
                <button key={p} onClick={() => setPage(p)}
                  className={`w-9 h-9 text-sm font-semibold rounded-xl transition tabular-nums ${p === page ? "bg-blue-500 text-white shadow-sm" : "border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-800 dark:hover:text-slate-100"}`}
                >{p}</button>
              ))}

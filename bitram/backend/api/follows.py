...
from db.models import User, Follow, Post, Strategy
...
async def follow_suggestions(
    ...
    """Suggest users to follow based on activity."""
    following_ids_stmt = select(Follow.following_id).where(Follow.follower_id == user.id)
    following_result = await db.execute(following_ids_stmt)
    ...
    stmt = (
        select(
            ...
            func.count(Strategy.id.distinct()).label("strategy_count"),
        )
        .outerjoin(Post, Post.user_id == User.id)
        .outerjoin(Strategy, and_(Strategy.user_id == User.id, Strategy.is_public == True))
        .where(User.is_active == True)
        .where(not_(User.id.in_(following_ids)))
        .group_by(User.id)
        .order_by(
            (func.count(Post.id.distinct()) + func.count(Strategy.id.distinct())).desc()
        )
        .limit(10)
    )
    rows = (await db.execute(stmt)).all()
    ...
    for r in rows:
        fc = (await db.execute(
            select(func.count()).select_from(Follow).where(Follow.following_id == r.id)
        )).scalar() or 0
        suggestions.append({
            "user_id": str(r.id),
            "nickname": r.nickname,

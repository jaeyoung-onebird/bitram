...
class CheckInResponse(BaseModel):
    success: bool
    points: int           # total points earned (base + bonus)
    base_points: int
    bonus: int            # streak bonus
    streak: int           # current streak after check-in
    checked_dates: list[str]  # current month's check-in dates
    message: str
...
async def get_attendance_status(
    ...
    return AttendanceStatusResponse(
        streak=current_streak,
        total_this_month=len(month_dates),
        checked_dates=month_dates,

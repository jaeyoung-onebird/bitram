    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 text-slate-500 dark:text-slate-400">
          ...
          {searchQuery && <p className="text-sm">다른 키워드로 검색해보세요.</p>}
        </div>
      ) : (
              ...
              <div className="flex items-center gap-2.5 mb-3">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-blue-500/10 flex items-center justify-center shrink-0">
                  <span className="text-lg sm:text-xl">
                    {community.icon || community.coin_pair || community.name.charAt(0)}
                  </span>
                </div>
                <div className="min-w-0">
                  ...
                  {community.coin_pair && (
                    <span className="text-xs text-slate-400 dark:text-slate-500">{community.coin_pair}</span>
                  )}
                </div>
              </div>

              {/* Description */}
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mb-3 flex-1">

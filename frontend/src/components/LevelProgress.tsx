...
export default function LevelProgress() {
  ...
  const isMaxLevel = info.points_next === null;
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-700/60 rounded-xl shadow-sm p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <LevelBadge level={info.level} name={info.name} color={info.color} size="md" />
        </div>
      ...
      {isMaxLevel && (
        <div className="text-xs font-medium text-amber-500">최고 레벨 달성!</div>
      )}
